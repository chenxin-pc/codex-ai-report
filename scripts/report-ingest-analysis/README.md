...
| `commands/import_reports_and_export_chunks.py` | 专用辅助脚本 | 面向东方财富本地研报目录，通过应用上传 API 导入 PDF，再从 MySQL 导出文档、切片和统计到 Excel。保留用于兼容已有批处理场景。 | 只在维护东方财富批量导入/导出流程时修改；通用流水线优先改 `commands/pipeline.py`。 |
...
优先使用 `commands/pipeline.py`。只有需要复现旧的东方财富批量导入导出路径时，才使用 `commands/import_reports_and_export_chunks.py`。
