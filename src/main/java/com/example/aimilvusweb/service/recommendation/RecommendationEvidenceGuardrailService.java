...
import com.example.aimilvusweb.service.taxonomy.QueryGuardrailDictionaryService;
...
public class RecommendationEvidenceGuardrailService {
     ...
     * @Date: 2026-05-24 00:45:00
     */
    private boolean hasEvidence(List<RetrievedChunk> chunks) {
        // 召回列表为空或 null 时没有证据。
        if (chunks == null || chunks.isEmpty()) {
     ...
     * @Date: 2026-05-24 00:45:00
     */
    private boolean isQueryRelevant(String query, List<RetrievedChunk> chunks) {
        // 规范化 query 用于字符重合度计算。
     ...
     * @Date: 2026-05-24 00:45:00
     */
    private EntitySignals extractEntitySignals(List<RetrievedChunk> chunks) {
        // 初始化股票代码集合。
     ...
     * @Date: 2026-05-24 00:45:00
     */
    private String combinedText(RetrievedChunk chunk) {
        // 获取召回结果的原始 Document。
