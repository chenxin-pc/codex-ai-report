...
/**
 * @Description: 入库向量批量写入器，负责按固定批次将 Document 写入 VectorStore。
 * @Logic: 将大列表按批量大小切分后逐批调用 VectorStore.add，调用方在全部写入成功后再回写 vectorStored。
 * @Param: 详见方法签名；无入参时为无。
...
public class ReportVectorBatchWriter {
    ...
    /**
     * @Description: 分批写入向量文档。
     * @Logic: 按 EMBEDDING_BATCH_SIZE 切分文档列表；任一批失败会抛出异常，调用方不会回写 vectorStored。
     * @Param: vectorStore 向量库实例；documents 待写入文档列表。
     * @Return: 无（仅向 VectorStore 写入数据）。
     * @author: cx
     * @Date: 2026-05-30 16:00:00
     */
    public void write(VectorStore vectorStore, List<Document> documents) {
