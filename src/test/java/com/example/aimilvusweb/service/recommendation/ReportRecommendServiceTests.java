...
/**
 * @Description: 推荐编排服务测试，验证流式事件顺序、输入拒绝、证据降级和缓存行为。
 * @Logic: 使用 mock 的检索、意图、护栏、模型和缓存依赖构造推荐链路，断言同步与 SSE 输出符合预期分支。
 * @Param: 无。
 * @Return: 无（测试类仅通过断言验证推荐服务行为）。
 * @author: cx
 ...
 * @Return: 详见返回类型；void 时为无（仅副作用）。
    ...
    private RetrievedChunk retrievedChunk() {
        ...
        return new RetrievedChunk(document, 0.9D, "child evidence", "parent evidence");
    }
